...
@interface LPCHotelNOViewController ()

@end

@implementation LPCHotelNOViewController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(row == 0){
        ...
        [button addTarget:self action:@selector(button_yanzheng:) forControlEvents:UIControlEventTouchUpInside];
        ...
        [button_one addTarget:self action:@selector(button_clanel:) forControlEvents:UIControlEventTouchUpInside];
...
#pragma mark  酒店未使用的验证核销
-(void)button_yanzheng:(UIButton *)sender{
    ...
    UIAlertAction *okAction = [UIAlertAction actionWithTitle:@"确认" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action){
        ...
        NSDictionary  *  dic = @{@"orderCode":_idString};
        ...
        [ZHJQHttpToll GET:LPCYANZHENGHEXIAO parameters:dic success:^(id responseObject) {
            ...
            if([[self headdic:dic_json] isEqualToString:@"0"]){
                ...
                [self MBShow:@"验证核销订单成功" backview:self.view];
...
#pragma mark  酒店未使用的取消订单
-(void)button_clanel:(UIButton *)sender{
